// dlopen the real furnace plugin and drive the value-semantic viz vtable end to end,
...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_FURNACE_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-furnace/build/plugins/furnace_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_FURNACE_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/furnace/test.fur")
}
...
fn furnace_viz_vtable() {
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping: test module {} not found (set RV_FURNACE_MODULE)", module_path().display());
            return;
        }
    };

    let service = PluginService::new("furnace-test", Vfs::new());
    ...
    assert!(st.caps & VizCaps::SCOPE.bits() != 0, "furnace must advertise Scope");
