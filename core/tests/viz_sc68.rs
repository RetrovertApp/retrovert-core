// dlopen the real sc68 plugin and drive the value-semantic viz vtable end to end,
// proving the scope-only model: advertises Scope but no PatternCells, leaves every
// pattern getter NULL, reports a dynamic scope channel count (YM tone or Paula
// voices depending on the tune), and returns non-silent scope only after an
// explicit set_scope_enabled (no hidden auto-on).
...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_SC68_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-sc68/build/plugins/sc68_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_SC68_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/sc68/shadow_of_the_beast.sc68")
}
...
fn sc68_viz_vtable() {
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping: test module {} not found (set RV_SC68_MODULE)", module_path().display());
            return;
        }
    };

    let service = PluginService::new("sc68-test", Vfs::new());
    ...
    assert!(st.caps & VizCaps::SCOPE.bits() != 0, "sc68 must advertise Scope");
