// dlopen the real zxtune plugin and drive the value-semantic viz vtable end to end,
// proving the scope-only model: advertises Scope but no PatternCells, leaves every
// pattern getter NULL, reports its 3 AY chip channels, and returns non-silent scope
// only after an explicit set_scope_enabled (no hidden auto-on).
...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_ZXTUNE_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-zxtune/build/plugins/zxtune_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_ZXTUNE_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/zxtune/isosentr.ayc")
}
...
fn zxtune_viz_vtable() {
    ...
    assert!(plugin.get_structure.is_some(), "zxtune must implement get_structure");
    assert!(plugin.get_scope_channels.is_some(), "zxtune must implement get_scope_channels");
    assert!(plugin.set_scope_enabled.is_some(), "zxtune must implement set_scope_enabled");
    assert!(plugin.get_scope_samples.is_some(), "zxtune must implement get_scope_samples");
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping zxtune live scope: module {} not found", module_path().display());
            return;
        }
    };

    let service = PluginService::new("zxtune-test", Vfs::new());
    ...
    // --- structure: scope-only — Scope, no PatternCells, 3 AY channels ---
    ...
    assert!(st.caps & VizCaps::SCOPE.bits() != 0, "zxtune must advertise Scope");
    ...
    assert_eq!(st.scope_channel_count, 3, "AY chip exposes 3 channels");

    // --- scope channels: 3, each named, mono ---
    let mut chans = vec![ChannelDesc { name: [0; 24], scope_width: 0 }; 3];
    let nc = (plugin.get_scope_channels.unwrap())(user_data, chans.as_mut_ptr(), chans.len() as u32);
    assert_eq!(nc, 3, "scope channel count must match structure");
