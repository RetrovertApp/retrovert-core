// dlopen the real adplug plugin and drive the value-semantic viz vtable end to end,
// proving the scope-only model: advertises Scope but no PatternCells, leaves every
// pattern getter NULL, reports a fixed OPL scope channel count, and returns non-silent
...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_ADPLUG_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-adplug/build/plugins/adplug_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_ADPLUG_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/adplug/test.hsc")
}
...
#[test]
fn adplug_viz_vtable() {
    let path = so_path();
    if !path.exists() {
        eprintln!("skipping: {} not built (run cmake --build in playback-adplug)", path.display());
        return;
    }
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping: test module {} not found (set RV_ADPLUG_MODULE)", module_path().display());
            return;
        }
    };

    let service = PluginService::new("adplug-test", Vfs::new());
    ...
    assert!(st.caps & VizCaps::SCOPE.bits() != 0, "adplug must advertise Scope");
    ...
    // --- scope-only contract: pattern getters are NULL, so no grid is ever requested ---
    ...
    assert!(ns > 0, "scope returned no samples after enable");
