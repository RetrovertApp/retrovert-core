// dlopen the real mdxmini plugin and drive the value-semantic viz vtable end to end,
...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_MDXMINI_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-mdxmini/build/plugins/mdxmini_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_MDXMINI_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/mdxmini/test.mdx")
}
...
fn mdxmini_viz_vtable() {
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping: test module {} not found (set RV_MDXMINI_MODULE)", module_path().display());
            return;
        }
    };

    let service = PluginService::new("mdxmini-test", Vfs::new());
    ...
    assert!(st.caps & VizCaps::SCOPE.bits() != 0, "mdxmini must advertise Scope");
