// dlopen the real pmdmini plugin and drive the value-semantic viz vtable end to end,
...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_PMDMINI_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-pmdmini/build/plugins/pmdmini_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_PMDMINI_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/pmdmini/test.m")
}
...
fn pmdmini_viz_vtable() {
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping: test module {} not found (set RV_PMDMINI_MODULE)", module_path().display());
            return;
        }
    };

    let service = PluginService::new("pmdmini-test", Vfs::new());
    ...
    assert!(st.caps & VizCaps::SCOPE.bits() != 0, "pmdmini must advertise Scope");
