...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_ORGANYA_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-organya/build/plugins/organya_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_ORGANYA_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/organya/test.org")
}
...
fn organya_viz_vtable() {
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping organya live scope: module {} not found", module_path().display());
            return;
        }
    };

    let service = PluginService::new("organya-test", Vfs::new());
    ...
    assert_eq!(st.scope_channel_count, 16, "organya has 8 melody + 8 percussion voices");
