...
fn ixalance_viz_vtable() {
    ...
    let mut st = VizInfo { caps: 0, scroll_mode: ScrollMode::PerChannel, pattern_channel_count: 0, scope_channel_count: 0, column_count: 0 };
    ...
    let nsc = (plugin.scope_channels.unwrap())(user_data, schans.as_mut_ptr(), schans.len() as u32);
    ...
    assert_eq!((plugin.tracker_channel_rows.unwrap())(user_data, rowbuf.as_mut_ptr(), nch as u32), 0);

    // --- cells, all channels: row -> channel -> column, raw + rendered text ---
    let mut cells = vec![PatternCell { raw: 0, text: [0; 16] }; rows * nch * 5];
    let got = (plugin.tracker_cells.unwrap())(user_data, -1, 0, rows as u32, cells.as_mut_ptr(), cells.len() as u32);
    ...
    let got1 = (plugin.tracker_cells.unwrap())(user_data, 0, 0, rows as u32, cells.as_mut_ptr(), cells.len() as u32);
    ...
    for ch in 0..nch {
        resumed = (plugin.scope_samples.unwrap())(user_data, ch as i32, scope.as_mut_ptr(), scope.len() as u32);
        if resumed > 0 {
            break;
        }
    }
    assert!(resumed > 0, "scope_enable(true) must resume capture");
