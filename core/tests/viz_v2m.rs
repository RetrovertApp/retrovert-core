// dlopen the real v2m plugin and drive the value-semantic viz vtable end to end,
// proving the scope-only model: advertises Scope but no PatternCells, leaves every
// pattern getter NULL, reports a per-synth-channel scope count, and returns
...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_V2M_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-v2m/build/plugins/v2m_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_V2M_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/v2m/test.v2m")
}
...
fn v2m_viz_vtable() {
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping: test module {} not found (set RV_V2M_MODULE)", module_path().display());
            return;
        }
    };

    let service = PluginService::new("v2m-test", Vfs::new());
    ...
    assert!(st.caps & VizCaps::SCOPE.bits() != 0, "v2m must advertise Scope");
