...
fn so_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_AUDIO_STREAM_SO") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../playback-audio_stream/build/plugins/audio_stream_playback.so")
}

fn module_path() -> PathBuf {
    if let Ok(p) = std::env::var("RV_AUDIO_STREAM_MODULE") {
        return PathBuf::from(p);
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../../replay_frontend/data/test_data/music/audio_stream/test.ogg")
}
...
fn audio_stream_viz_vtable() {
    ...
    let module = match std::fs::canonicalize(module_path()) {
        Ok(p) => p,
        Err(_) => {
            eprintln!("skipping: test module {} not found (set RV_AUDIO_STREAM_MODULE)", module_path().display());
            return;
        }
    };

    let service = PluginService::new("audio_stream-test", Vfs::new());
